...
function buildPopup(asset, onEdit, onDelete) {
  ...
  const condColor =
    { Good: "#0fba7d", Defective: "#f0a429", Critical: "#f04545" }[
      asset.condition
    ] || "#888";
  ...
  el.querySelector("#pop-delete").addEventListener("click", () =>
    onDelete(asset.id),
  );
  return el;
}
...
export default function MapView({ assets, onMapClick, onEdit, onDelete }) {
  ...
  useEffect(() => {
    ...
    assets.forEach((asset) => {
      const icon = isOverdue(asset)
        ? ICONS.Overdue
        : ICONS[asset.condition] || ICONS.Good;
      const marker = L.marker([asset.latitude, asset.longitude], { icon });
      marker.bindPopup(() => buildPopup(asset, onEdit, onDelete), {
        minWidth: 226,
      });
  ...
  return (
    <div className="relative h-full w-full">
      ...
      <div
        className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[500] pointer-events-none
